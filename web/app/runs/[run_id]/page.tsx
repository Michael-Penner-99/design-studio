...
export default async function RunDetailPage({ params }: { params: Params }) {
  ...
  if (!synthetic) {
    ...
    if (!spec) notFound();
    synthetic = {
      run_id: spec.run_id, slug: null, mode: spec.mode,
      url: spec.url ?? null, business_name: spec.business_name ?? null,
      started_at: spec.submitted_at, updated_at: null, status: "queued",
      current_phase: 0, phases: {}, outputs: null, halt_reason: null, halt_phase: null,
    };
  }
  const run = synthetic;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_320px]">
        <section>
          <h2 className="mb-3 text-sm font-semibold uppercase tracking-wider text-muted">Phase progress</h2>
          <PhaseStepper run={run} poll={true} />
        </section>
        <section className="rounded-lg border border-white/10 bg-surface p-4">
          <ActivityFeed runId={run.run_id} slug={run.slug ?? null} isActive={isActive} />
        </section>
      </div>

      {run.status === "completed" && run.outputs ? <OutputsPanel outputs={run.outputs} /> : null}
      {run.status === "halted" ? <ResumeForm run={run} formToken={formToken} /> : null}
    </div>
  );
}
...
function SpecPanel({ run }: { run: RunStatus }) {
  return (
    <section className="card">
      <h2 className="mb-3 text-sm font-semibold uppercase tracking-wider text-muted">Spec</h2>
      <dl className="grid grid-cols-1 gap-y-2 text-sm md:grid-cols-2">
        ...
        {run.business_name ? <Row label="Business" value={run.business_name} /> : null}
        {run.slug ? <Row label="Slug" value={run.slug} mono /> : null}
        <Row label="Started" value={formatDate(run.started_at)} />
        {run.updated_at ? <Row label="Updated" value={formatDate(run.updated_at)} /> : null}
      </dl>
    </section>
  );
}
...
function OutputsPanel({ outputs }: { outputs: NonNullable<RunStatus["outputs"]> }) {
  return (
    <section className="rounded-lg border border-emerald-500/30 bg-emerald-500/5 p-5">
      <h2 className="mb-3 text-sm font-semibold uppercase tracking-wider text-emerald-300">Deliverables</h2>
      <div className="flex flex-wrap gap-3">
        {outputs.site_url ? <a href={outputs.site_url} target="_blank" rel="noreferrer" className="btn-primary">Open contractor site →</a> : null}
        {outputs.sales_walkthrough_url ? <a href={outputs.sales_walkthrough_url} target="_blank" rel="noreferrer" className="btn-secondary">Open sales walkthrough →</a> : null}
        {outputs.proposal_pdf_path ? <span className="text-xs text-muted self-center font-mono">Proposal: {outputs.proposal_pdf_path}</span> : null}
      </div>
    </section>
  );
}
